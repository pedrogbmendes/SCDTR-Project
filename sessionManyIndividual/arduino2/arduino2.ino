...
class consensus_class{
  ...
  class controller{
    public:
      ...
      void individual_controller();
    private:
      int feedback_control();
      float calculate_t_const(int);
      int feedforward_control(float);
      float simulator(float, float, unsigned long);
  };
...
void consensus_class::concensus(float lumi_desire){
    ...
    for(int h=0; h<50; h++){
      ...
      send_msg(SEND_RESULT, my_address, node.d[0], node.d[1], 1);
...
void receive_msg(int numBytes){
  ...
  if (msg_recv[0] != 'C' || msg_recv[0] != 'I' || msg_recv[0] != 'F') {
...
**************************************************************************/
float controller::calculate_t_const(int pwm_value)
{
...
**************************************************************************/
int controller::feedforward_control(float ill_desire){

  float pwm_des = ill_desire / gain[my_address-1];
  return int(pwm_des);
}
...
**************************************************************************/
float controller::simulator(float ill_desire, float v_ini, unsigned long t_ini){
  float tau, v_f, y;
  float pwm_des = ill_desire / gain[my_address-1];
...
**************************************************************************/
void controller::individual_controller(){
  ...
  u_con = flag_ff * feedforward_control(ill_des);
