...
volatile bool flag_new_node = false;
volatile bool flag_stop_wait = false;
int orig_addr = 0;
...
//time
unsigned long t_init, t_ini_wait;
...
void setup() {
  ...
  t1 = t_init;
   ...
   while(millis() <= 5000 && !flag_calib && !flag_stop_wait){
    if(flag_new_node){
      char str_send[5];
      sprintf(str_send, "%s%d", NODE_HERE, my_address);
...
void INIT_cali::new_node() {
  ...
  sprintf(str_send, "%s%d",NEW_NODE , my_address);
  Wire.beginTransmission(bus_add);
  Wire.write(str_send);
  Wire.endTransmission();
  ...
  while(millis() - t_ini_wait < 1000 && !flag_stop_wait){
    
  }
  Serial.println(flag_stop_wait);
  if(flag_stop_wait){
     calib.init_calibration();
     flag_stop_wait = false;
  }
}
